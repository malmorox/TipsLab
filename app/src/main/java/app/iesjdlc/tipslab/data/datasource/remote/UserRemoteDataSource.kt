...
class UserRemoteDataSource @Inject constructor(
    ...
    override suspend fun existsByEmail(email: String): Boolean =
        !db.collection("users").whereEqualTo("email", email).get().await().isEmpty

    override suspend fun existsByUsername(username: String): Boolean =
        !db.collection("users").whereEqualTo("username", username).get().await().isEmpty
    ...
    override suspend fun update(id: String, dto: UserDto) {
        db.collection("users").document(id).set(dto).await()
    }
}