...
class UserRemoteDataSource @Inject constructor(
    ...
    override suspend fun existsByEmail(email: String): Boolean =
        !db.collection(DBConstants.Remote.USERS_COLLECTION)
            .whereEqualTo(DBConstants.Remote.EMAIL_FIELD, email)
            .get().await().isEmpty

    override suspend fun existsByUsername(username: String): Boolean =
        !db.collection(DBConstants.Remote.USERS_COLLECTION)
            .whereEqualTo(DBConstants.Remote.USERNAME_FIELD, username)
            .get().await().isEmpty
    ...
    override suspend fun getEmailByUsername(username: String): String? =
        db.collection(DBConstants.Remote.USERS_COLLECTION)
            .whereEqualTo(DBConstants.Remote.USERNAME_FIELD, username)
            .get().await()
            .documents.firstOrNull()?.getString(DBConstants.Remote.EMAIL_FIELD)
    ...
    override suspend fun getByIds(ids: List<String>): Map<String, UserDto> {
        ...
        return db.collection(DBConstants.Remote.USERS_COLLECTION)
